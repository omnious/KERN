...
class KERN_oms(nn.Module):
    def __init__(self, conf, adj=None):
        super(KERN_oms, self).__init__()
        self.conf = conf
        ...
        if self.conf["dataset"] == "fit" or self.conf["dataset"] == "omnious":
            self.location_embeds = nn.Embedding(self.conf["location_num"], self.conf["feat_size"])
            self.segment_embeds = nn.Embedding(self.conf["segment_num"], self.conf["feat_size"])
            self.target_age_embeds = nn.Embedding(self.conf["target_age_num"], self.conf["feat_size"])
        ...
        self.location_segment_target_age_agg = nn.Linear(conf["feat_size"] * 3, conf["feat_size"])
    ...
    def predict(self, each_cont):
        input_seq, output_seq, grp_id, ele_id, _, location_id, segment_id, target_age_id = each_cont

        grp_id = grp_id.squeeze(1)  # [batch_size]
        location_id = location_id.squeeze(1)  # [batch_size]
        segment_id = segment_id.squeeze(1)  # [batch_size]
        target_age_id = target_age_id.squeeze(1)  # [batch_size]
        ele_id = ele_id.squeeze(1)  # [batch_size]

        location_embed = self.location_embeds(location_id)  # [batch_size, feat_size]
        segment_embed = self.segment_embeds(segment_id)  # [batch_size, feat_size]
        target_age_embed = self.target_age_embeds(target_age_id)  # [batch_size, feat_size]
        ori_ele_embed = self.ele_embeds(ele_id)  # [batch_size, feat_size]

        grp_embed = self.location_segment_target_age_agg(torch.cat([location_embed, segment_embed, target_age_embed], dim=1))
        ...
        if self.conf["use_grp_embed"] is True:
            enc_input_seq = torch.cat([enc_grp_embed, enc_ele_embed, enc_time_embed, input_seq[:, :, 1].unsqueeze(-1)],
                                      dim=-1)  # [batch_size, enc_seq_len, enc_input_size]
        else:
            enc_input_seq = torch.cat([enc_ele_embed, enc_time_embed, input_seq[:, :, 1].unsqueeze(-1)],
                                      dim=-1)  # [batch_size, enc_seq_len, enc_input_size]
        enc_input_seq = enc_input_seq.permute(1, 0, 2)  # [enc_seq_len, batch_size, enc_input_size]

        enc_outputs, (enc_hidden, enc_c) = self.encoder(
            enc_input_seq)  # outputs: [seq_len, batch_size, hidden_size], hidden: [1, batch_size, hidden_size]
        ...
        if self.conf["use_grp_embed"] is True:
            dec_input_seq = torch.cat([dec_grp_embed, dec_ele_embed, dec_time_embed],
                                      dim=-1)  # [batch_size, dec_seq_len, dec_input_size]
        else:
            dec_input_seq = torch.cat([dec_ele_embed, dec_time_embed],
                                      dim=-1)  # [batch_size, dec_seq_len, dec_input_size]
        ...
        dec_output_feat, _ = self.decoder(dec_input_seq, (
        dec_init_hidden.contiguous(), dec_init_c.contiguous()))  # outputs: [seq_len, batch_size, hidden_size*2]
        ...
        return enc_loss, dec_loss, dec_pred, enc_hidden.squeeze(
            0), enc_output_feat, dec_output_feat  # [batch_size, hidden_size], [batch_size, enc_seq_len-1, hidden_size], [batch_size, seq_len, hidden_size*2]

    def forward(self, self_cont, close_cont, far_cont, close_score, far_score):
        self_enc_loss, self_dec_loss, self_pred, self_enc_hidden, self_enc_output, self_dec_output = self.predict(
            self_cont)
        close_enc_loss, close_dec_loss, close_pred, close_enc_hidden, close_enc_output, close_dec_output = self.predict(
            close_cont)
